...
- [ ] 1.2 为成功 envelope、精确六字段、空目录、成员 allowlist、未知 platform/status string 和 ID 升序建立失败优先的 contract tests，并验证测试在实现前按预期失败
...
- [ ] 2.1 新增不可变 SQL migration，创建 NOLOGIN `relay_directory_definer` owner、LOGIN `relay_directory_db_reader` runtime role 和固定 search_path 的 SECURITY DEFINER function，并用 role/migration tests 验证身份属性
- [ ] 2.2 撤销 PUBLIC 权限并只向 `relay_directory_db_reader` 授予精确 function execute，使用 privilege test 验证直接读取 `accounts`、`credentials` 和其他业务表失败
- [ ] 2.3 在数据库 projection 中实现 `base_url` JSON string type 与 4096-byte ceiling，并用 SQL test 验证缺失、非 string、4096/4097-byte 值只按契约返回有界 scalar 或 NULL
- [ ] 2.4 实现单 statement 的 source time、空集合 sentinel、成员过滤、`id ASC` 和 `LIMIT 10001` bounded overflow sentinel，并用 Repository test 验证同一 snapshot 与第 10,001 行整体失败
- [ ] 2.5 给专用 projection 增加 `deleted_at`、已知排除类型和未知类型测试，并验证 persistent status 与 scheduler/runtime 字段变化不影响成员资格
...
- [ ] 3.3 增加启动配置校验：enabled 时 runtime 只要求 token decoded length 至少 32 bytes 和有效专用数据库配置，并用 table test 验证缺失/过短值 fail-closed
...
- [ ] 6.3 更新 Gateway config hooks 与 Ops 部署文档，明确 Gateway 负责 route/auth/default-disabled，Ops 负责 CSPRNG 256-bit token、TLS、management ACL、public-ingress deny 和 Secret deployment，并检查未满足前置条件时保持 disabled
