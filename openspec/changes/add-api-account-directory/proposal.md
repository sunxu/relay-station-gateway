...
- 新增独立 service token 鉴权、精确 route/method 授权、TLS 与 restricted management network 部署约束，以及最小数据库投影和 Secret/log/error 脱敏要求。
- 新增完整性、响应大小、Account 数、数据库查询时间、HTTP 总时间、并发和单 identity 速率 hard limits；任何失败或超限都返回明确 non-2xx，禁止部分成功、截断或 first-N。
...
- **部署与安全**：需要独立 `relay_control_reader` token、TLS、management network ACL，以及只允许安全投影读取的数据库身份；Control 仍不得直连 Gateway PostgreSQL。
