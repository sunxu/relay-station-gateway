...
- 新增独立 service token 鉴权：Ops 使用 CSPRNG 生成至少 32 random bytes 并采用 unpadded Base64URL wire format；同时增加精确 route/method 授权、TLS 与 restricted management network 部署约束，以及复用 Gateway 现有数据库连接、带 4096-byte `credentials.base_url` ceiling 的单条最小 raw SQL projection 和 Secret/log/error 脱敏要求。
...
- **后端**：后续实现预计影响 Gateway internal route、专用鉴权、配置/Secret、复用现有连接的数据库 allowlist projection、URL sanitization、限流/并发控制、handler/service/repository 接线和相应测试；不新增数据库变更脚本、schema object、访问身份、credential 或 connection pool。
- **身份与安全**：HTTP identity 为 `relay_control_reader`。Gateway 现有 DB identity 本身已有业务权限；Directory 的最小披露由独立 raw SQL allowlist projection、模块依赖隔离和测试保证，而不是新增 PostgreSQL privilege boundary。Control 仍不得直连 Gateway PostgreSQL 或获得数据库 credential。
...
- **回滚**：可通过撤销 internal route 暴露、停用独立 reader token/网络 ACL 和回退应用代码来移除能力；没有 Directory 数据库对象、credential 或数据变更需要回滚。
